...
const getAuditLabel = (n: any): string => {
  // Prefer the title if it's specific and descriptive
  if (n.title) {
    ...
    if (n.title.includes("Expansion") || n.title.includes("Domain Expansion")) return "Domain Expansion";
...
// Parse a markdown message string into React-renderable segments
const parseDescription = (msg: string) => {
  ...
  let lastIndex = 0;
  let match;
  // Clean text only (no ** markers — we'll bold the labels separately)
...
export default function AdminBusinessDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm(`Are you sure you want to permanently delete "${business?.name}"? This cannot be undone.`)) return;
    try {
  ...
  // Build combined history deduplicating notification pairs (super_admin + tourism_admin)
  const seen = new Set<string>();
  const dedupedNotifications = notifications.filter((n: any) => {
    if (n.type === "internal_chat") return false; // exclude chat messages from audit log
      ...
      <main className="max-w-6xl mx-auto px-3 md:px-4 lg:px-5 py-10 lg:py-20">
        ...
        <div className="bg-white rounded-[60px] p-10 md:p-16 shadow-2xl shadow-foreground/5 border border-foreground/[0.03] mb-10 animate-fade-in">
          ...
          {business.description && (
            <p className="mt-8 text-foreground/50 font-medium italic leading-relaxed border-l-4 border-primary/20 pl-6 max-w-3xl">
              "{business.description}"
            </p>
          )}
        </div>
