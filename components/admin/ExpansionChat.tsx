...
export default function ExpansionChat({ notificationId, currentRole, initialDiscussion }: { notificationId: string, currentRole: string, initialDiscussion?: Message[] }) {
  ...
  const fetchDiscussion = async () => {
    ...
    try {
      ...
      const currentReq = data.requests.find((r: any) => r._id === notificationId);
      setMessages(currentReq?.discussion || []);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      if (process.env.NEXT_PUBLIC_PUSHER_KEY) {
        channel = pusherClient.subscribe(`admin-notifications-${currentRole}`);
    ...
    return () => {
      if (channel) {
        try {
          pusherClient.unsubscribe(`admin-notifications-${currentRole}`);
        } catch (e) {}
      }
      clearInterval(interval);
    };
  }, [notificationId, currentRole]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/business/category-request`, {
        ...
        body: JSON.stringify({ notificationId, message: msgContent }),
      });
      if (res.ok) {
        fetchDiscussion();
      } else {
